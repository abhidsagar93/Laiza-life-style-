...
module.exports = async (req, res) => {
  ...
  if (!order.shipping_address || !order.shipping_pincode) {
    return json(res, 400, {
      error: 'This order has no shipping address on file. This can happen for orders placed before shipping details were tracked — add the address manually in Shiprocket instead.'
    });
  }
  ...
  try {
    const createRes = await fetch(`${SHIPROCKET_BASE}/orders/create/adhoc`, {
      ...
      body: JSON.stringify({
        ...
        billing_customer_name: order.shipping_full_name || 'Customer',
        billing_last_name: '',
        ...
        billing_phone: (order.shipping_phone || '').replace(/\D/g, '').slice(-10),
  ...
  try {
    const rateRes = await fetch(
      `${SHIPROCKET_BASE}/courier/serviceability/?pickup_postcode=${pickupPincode}&delivery_postcode=${order.shipping_pincode}&weight=${Math.max(totalWeightKg, 0.05)}&cod=${isCod ? 1 : 0}`,
      { headers: srHeaders }
    );
