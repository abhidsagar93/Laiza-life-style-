...
module.exports = async function handler(req, res) {
  ...
  const hashVerified = !!receivedHash && receivedHash === expectedHash;

  // Log this attempt no matter what happens next — this is the diagnostic trail.
  const { data: matchedOrder } = hashVerified && payload.udf1
    ? await supabaseAdmin.from('orders').select('order_number').eq('id', payload.udf1).maybeSingle()
    : { data: null };

  await supabaseAdmin.from('payu_webhook_attempts').insert({
    hash_verified: hashVerified,
    status_field: payload.status || null,
    txnid: payload.txnid || null,
    order_id_from_udf1: payload.udf1 || null,
    matched_order_number: matchedOrder?.order_number || null,
    raw_payload: payload
  });
